...
class EmailService:
    def __init__(self):
        ...
        self.db_url = "postgresql://postgres:uqyVedPrLdr6sa38@db.tdzxglexkgqxnguaetwv.supabase.co:5432/postgres"
    ...
    def send_verification_code(self, email, action):
        ...
        try:
            ...
            # Salvar no banco
            if not self.save_verification_code(email, code, action):
                return False, "Erro ao salvar código"
            
            # Enviar email (real ou simulado)
            if self.dev_mode:
                # Modo desenvolvimento - apenas simular
                print(f"\n{'='*50}")
                print(f"EMAIL SIMULADO ENVIADO")
                print(f"{'='*50}")
                print(f"Para: {email}")
                print(f"Ação: {action}")
                print(f"Código: {code}")
