...
DB_URL = "postgresql://postgres:uqyVedPrLdr6sa38@db.tdzxglexkgqxnguaetwv.supabase.co:5432/postgres"
...
def process_ocr():
    ...
    try:
        ...
        # Gerar CPF de exemplo para demonstração
        cpf_exemplo = f"{random.randint(100, 999)}.{random.randint(100, 999)}.{random.randint(100, 999)}-{random.randint(10, 99)}"
        
        return jsonify({
            'success': True,
            'cpf': cpf_exemplo,
            'confidence': random.randint(85, 99),
            'message': 'CPF extraído com sucesso (modo demonstração)'
        }), 200
...
def execute_automation():
    ...
    try:
        ...
        try:
            # Usar automação simples em vez do Playwright
            import sys
            sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))
            from automation_simples import executar_automacao_simples
            
            # Executar automação simples
            resultado = executar_automacao_simples(test_data)
...
def register():
    ...
    try:
        ...
        success, message = auth_system.check_email_exists(data['email'])
        if not success:
            return jsonify({'error': message}), 400
        
        # Enviar código de verificação
        success, result = send_verification_code_simulation(data['email'], 'register')
        if success:
            return jsonify({
                'success': True,
